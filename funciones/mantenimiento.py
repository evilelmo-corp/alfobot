...
def guardadoinputs(message,tokens_limpios):
	sep=";--;"
	with open(f"datos/inputs_espia.csv","a") as fh:
...
def actualizarpickles():
	...
	'''
	pick=pd.read_pickle(f'datos/rayo_lemmatizador')
	with open(f'datos/a_pickelizar.txt') as apc:
		apk=apc.read()
	l_apk=apk.split(";")
	for tup in l_apk[:-1]:
		l_tup=tup.split(",")
	...
	pick.to_pickle(f'datos/rayo_lemmatizador')
	remove(f'datos/a_pickelizar.txt')
