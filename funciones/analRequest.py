...
async def listalizador(frase, indice):    
...
async def lemmatizer(palabra):
    with open('rayo_lemmatizador' , 'rb') as f:
        rl = pickle.load(f)
    try:
        await rl[rl['Form']==palabra].iloc[0,1]
    except:
        await 'No lemma'

async def analizador(frase)
	with open('columnas', 'rb') as files:
	    colum=pickle.load(files)
